...
data_state=data_state[,c("Province/State","Country/Region","Deaths","Confirmed")]
...
for(i in 2:length(dts)){
  ...
  if(i<61){
    data_tmp=data_tmp[,c("Province/State","Country/Region","Deaths","Confirmed")]
    data_tmp$date=dts[i]
  }else{
    data_tmp=data_tmp[,c("Province_State","Country_Region","Deaths","Confirmed")]
    data_tmp$date=dts[i]
...
output_state_death=spread(agg_state,date,Deaths)
output_state_death=output_state_death[is.element(output_state_death$Province.State,state_list),]
output_state_death[is.na(output_state_death)]=0

N=ncol(output_state_death)
output_tmp=output_state_death
output_state_death[,2:(N-1)]=output_state_death[,3:N]-output_state_death[,2:(N-1)]
output_state_death=output_state_death[,1:(N-1)]

write.csv(output_state,"data_sir/state_confirmed_counts.csv",row.names=F)
write.csv(output_state_death,"data_sir/state_deaths_counts.csv",row.names=F)
